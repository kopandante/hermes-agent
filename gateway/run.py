...
class GatewayRunner:
    ...
    def __init__(self, config: Optional[GatewayConfig] = None):
        ...
        self._gateway_pending: Dict[str, Dict[str, Any]] = {}  # clarify/sudo pending responses
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        # Intercept text responses to pending clarify/sudo prompts
        if self._gateway_pending:
            for _pid, _entry in list(self._gateway_pending.items()):
                if _entry.get("awaiting_text") and _entry.get("chat_id") == source.chat_id:
                    text = (event.text or "").strip()
                    if text:
                        _entry["response"] = text
                        _entry["awaiting_text"] = False
                        _entry["event"].set()
                        return f"> {text}"
                    break
    ...
    async def _run_agent(
        ...
        def run_sync():
            ...
            # Clarify callback — send inline buttons on Telegram, numbered list elsewhere.
            # Blocks the agent thread until the user responds or timeout.
            def _clarify_sync(question: str, choices: list = None) -> str:
                _cid = f"clr:{session_id[-8:]}:{int(time.time())}"

                async def _send():
                    from gateway.platforms.telegram import TelegramAdapter
                    if isinstance(_status_adapter, TelegramAdapter) and choices:
                        from telegram import InlineKeyboardButton, InlineKeyboardMarkup
                        rows = [[InlineKeyboardButton(c, callback_data=f"clarify:{_cid}:{i}")]
                                for i, c in enumerate(choices)]
                        rows.append([InlineKeyboardButton("Other", callback_data=f"clarify:{_cid}:other")])
                        return await _status_adapter._bot.send_message(
                            chat_id=int(_status_chat_id), text=f"**{question}**",
                ...
                try:
                    asyncio.run_coroutine_threadsafe(_send(), _loop_for_step).result(timeout=15)
                except Exception as exc:
                    logger.warning("clarify send failed: %s", exc)
                    return "(clarify prompt failed to send)"

                pending = self._gateway_pending
                evt = threading.Event()
                pending[_cid] = {"event": evt, "response": None, "choices": choices, "chat_id": _status_chat_id}
                evt.wait(timeout=120)
                answer = pending.pop(_cid, {}).get("response") or "(no response)"
                return answer

            agent.clarify_callback = _clarify_sync

            # Sudo callback — ask for password via text message.
            def _sudo_sync() -> str:
                _sid = f"sudo:{session_id[-8:]}:{int(time.time())}"
                try:
                    asyncio.run_coroutine_threadsafe(
                        _status_adapter.send(_status_chat_id,
                            "*Sudo password required*\n_Reply with password (or empty to skip):_",
                            metadata=_status_thread_metadata),
                        _loop_for_step,
                    ).result(timeout=10)
                except Exception:
                ...
                pending = self._gateway_pending
                evt = threading.Event()
                pending[_sid] = {"event": evt, "response": None, "awaiting_text": True, "chat_id": _status_chat_id}
                evt.wait(timeout=45)
                return pending.pop(_sid, {}).get("response") or ""

            from tools.terminal_tool import set_sudo_password_callback
            set_sudo_password_callback(_sudo_sync)

            # Mark as gateway session so approval.py prompts instead of auto-bypassing
